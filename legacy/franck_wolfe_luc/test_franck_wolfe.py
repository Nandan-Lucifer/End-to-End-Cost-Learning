...
def construct_cost_matrix(G1, G2, cost):
    ...
    C2 = cost[3]*torch.tensor(np.array([[k != l and G1.has_edge(k//(m+1), l//(m+1)) ^ G2.has_edge(
        k % (m+1), l % (m+1)) for k in range((n+1)*(m+1))] for l in range((n+1)*(m+1))]))

    D = torch.zeros((n+1)*(m+1))
    D[n*(m+1):] = costs[1]
    D[n*(m+1)+m] = 0
    D[np.array([i*(m+1)+m for i in range(n)])] = costs[1]
    D[[k for k in range(n*(m+1)) if k % (m+1) != m]] = costs[0]*torch.tensor(
        [(G1.nodes[k//(m+1)] != G2.nodes[k % (m+1)]) for k in range(n*(m+1)) if k % (m+1) != m])
...
# 0 node subs, 1 nodeIns/Del, 2 : edgeSubs, 3 edgeIns/Del
